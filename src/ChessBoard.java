public class ChessBoard {
    ...
    public boolean isBadPosition(int line, int column) {
        return line < 0 || line > 7 || column < 0 || column > 7;
    }

    public boolean isPathFree(int lBegin, int cBegin, int lEnd, int cEnd) {
        if (isBadPosition(lBegin, cBegin) || isBadPosition(lEnd, cEnd))
            return false;
        if (lBegin != lEnd && cBegin != cEnd &&
                lBegin + cBegin != lEnd + cEnd &&
                lBegin - cBegin != lEnd - cEnd)
            return false;
        int dLine = (int) java.lang.Math.signum(lEnd - lBegin);
        int dColumn = (int) java.lang.Math.signum(cEnd - cBegin);
        int line = lBegin + dLine;
        int column = cBegin + dColumn;

        while (line != lEnd || column != cEnd) {
            if (board[line][column] != null) return false;
            line += dLine;
            column += dColumn;
        }
